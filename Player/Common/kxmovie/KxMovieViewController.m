...
@property (nonatomic, strong) UIView *controllerView; // 控制按钮界面
...
- (void)loadView
{
    ...
    self.controllerView = [[UIView alloc] initWithFrame:bounds];
...
- (void) handlePan: (UIPanGestureRecognizer *) sender
{
    if (sender.state == UIGestureRecognizerStateEnded) {
        ...
        const CGFloat sp = MAX(0.1, log10((double)(vt.x)) - 1.0);
        const CGFloat sc = (double)(pt.x) * 0.33 * sp;
        if (sc > 10) {
...
//    CGRect frame = CGRectMake(0, 20, bounds.size.width, bounds.size.width * 0.75);
